...
from utils import get_residual_features, get_matched_ref_features,get_matched_ref_features_topk
    ...
    for idx, batch in enumerate(test_loader):
        ...
        with torch.no_grad():
            if args.backbone == 'wide_resnet50_2':
                ...
                features_ad = [adapters[i](features[i]) for i in range(len(features))]
                combined_features = features + features_ad    
                mfeatures = get_matched_ref_features_topk(features_ad, ref_features,args.rank)
